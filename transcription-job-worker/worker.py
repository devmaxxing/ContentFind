...
# Add Azure Speech API configuration
AZURE_SPEECH_KEY = os.getenv("AZURE_SPEECH_KEY")
AZURE_SPEECH_REGION = os.getenv("AZURE_SPEECH_REGION")
AZURE_SPEECH_URL = f"https://{AZURE_SPEECH_REGION}.api.cognitive.microsoft.com/speechtotext/transcriptions:transcribe?api-version=2024-11-15"
...
# Load models - remove NeMo ASR
vad_model = load_silero_vad()

DATABASE_URL = os.getenv("DATABASE_URL")


# Replace D1 functions with Postgres implementations
    ...
    with psycopg.connect(DATABASE_URL, row_factory=dict_row) as conn:
        with conn.cursor() as cur:
            cur.execute(
                ...
                SET job_state = %s
                FROM selected_job s
                WHERE t.platform_id = s.platform_id 
    ...
    with psycopg.connect(DATABASE_URL, row_factory=dict_row) as conn:
        with conn.cursor() as cur:
            cur.execute(
                ...
                RETURNING *;
            """,
                (new_status, platform_id, content_id),
            )
...
def transcribe_with_azure(audio_path):
    ...
    response = requests.post(AZURE_SPEECH_URL, headers=headers, files=files)
    if response.status_code != 200:
        raise Exception(f"Azure transcription failed: {response.text}")

    result = response.json()
    phrases = []

    # Extract each phrase with its timing
    for phrase in result["phrases"]:
        phrases.append(
            {
                "start": phrase["offsetMilliseconds"] / 1000,  # Convert to seconds
                "duration": phrase["durationMilliseconds"] / 1000,  # Convert to seconds
                "text": phrase["text"],
            }
        )
...
def process_job(job):
    ...
    # Extract speech chunks and transcribe them
    transcriptions = []
    for i, chunk in enumerate(speech_chunks):
        ...
        try:
            phrases = transcribe_with_azure(chunk_path)
            # Adjust timestamps relative to chunk start
            chunk_start = speech_timestamps[i]["start"] / 16000  # Convert to seconds
            for phrase in phrases:
                transcriptions.append(
                    [chunk_start + phrase["start"], phrase["duration"], phrase["text"]]
                )
        finally:
